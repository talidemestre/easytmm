...
for im=1:numExpTend % for each month
  ...
  if numExpTend>1
	...
	save(['Aexp' sprintf('%02d',runId) suff],'Aexp','-v7.3')
	clear Aexp
...
for im=1:numImpTend % for each month
  ...
  save(['Aimp' sprintf('%02d',runId) suff],'Aimp','-v7.3')
  clear Aimp
end % end implicit matrix

!echo donetrans > donetrans
!echo alldone > alldone
% !mv Aimp*.mat Aexp*.mat matrix_output
eval(['!mv Aimp*.mat Aexp*.mat ' outdir]);
