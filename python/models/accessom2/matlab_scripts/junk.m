function [A1,A2] = assemble_transport_matrix(Ir,base,flag,Itend,tol,matType,maxNumTend)
...
elseif flag==11  % same as 9 (load matrices of type matType and average in time), but for new file format
  ...
  for j=1:length(Ir)
	...
	for km=1:numTend
	  ...
	  if matType==1
		fn=['Aexp' int2str(i) suff];
		load([base '/Runs/' 'Job' int2str(i) '/' fn],'Aexp')
		if tol>0
		  [ii,jj,aa]=find(Aexp);
		  k=find(abs(aa)>=tol);
		  ii=ii(k); jj=jj(k); aa=aa(k);
	  ...
	  elseif matType==2
		fn=['Aimp' int2str(i) suff];
		load([base '/Runs/' 'Job' int2str(i) '/' fn],'Aimp')
		if tol>0
		  [ii,jj,aa]=find(Aimp);
		  k=find(abs(aa)>=tol);
		  ii=ii(k); jj=jj(k); aa=aa(k);
