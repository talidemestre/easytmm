...
elseif userStep==-1;%plotting
    ...
    title0='annual global overturning at \approx 1000m depth'; aa=axis; aa()
    ...
    myCaption={'annual atlantic overturning at select latitudes at $\\approx$ 1000m depth'};
    if addToTex; write2tex(fileTex,2,myCaption,gcf); end;
    end;
    ...
    set(gca,'FontSize',14); grid on; axis([-90 90 0 3]);
    ...
    set(gca,'FontSize',14); grid on; axis([-90 90 -1.5 1.5]);
    if doAnomalies; aa=axis; aa(3:4)=scaleAnom*[-1 1]*0.1; axis(aa); end;
    title('Meridional Fresh Water Transport (in Sv)');
    ...
    set(gca,'FontSize',14); grid on; axis([-90 90 0 40]);
    ...
    if doAnomalies; cc=scaleAnom*[-1:0.1:1]*0.05; end;
    figureL; pcolor(x,y,fld); shading flat; axis([TT(1) TT(end) -90 90]);
    gcmfaces_cmap_cbar(cc); title('Meridional Fresh Water Transport (in Sv)');
    ...
    txt='Indonesian Throughflow (>0 toward Indian)';
    ylim=[5 25];
