...
if Xis2D
  lon_varid = netcdf.defVar(ncid,xname,'float',[lon_dimid lat_dimid]);     
else
  lon_varid = netcdf.defVar(ncid,xname,'float',lon_dimid);     
end
if Xis2D
  lat_varid = netcdf.defVar(ncid,yname,'float',[lon_dimid lat_dimid]);     
else
  lat_varid = netcdf.defVar(ncid,yname,'float',lat_dimid);     
end
if haveZ
  dep_varid = netcdf.defVar(ncid,zname,'float',dep_dimid);
end  
if haveTime
  time_varid = netcdf.defVar(ncid,tname,'float',time_dimid);
  if haveZ
    data_varid1 = netcdf.defVar(ncid,varName,'float',[lon_dimid lat_dimid dep_dimid time_dimid]);
  else
    data_varid1 = netcdf.defVar(ncid,varName,'float',[lon_dimid lat_dimid time_dimid]);
  end  
else
  if haveZ
    data_varid1 = netcdf.defVar(ncid,varName,'float',[lon_dimid lat_dimid dep_dimid]);
  else
    data_varid1 = netcdf.defVar(ncid,varName,'float',[lon_dimid lat_dimid]);
  end    
end
